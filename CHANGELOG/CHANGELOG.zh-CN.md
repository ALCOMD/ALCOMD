...
- 新增默认启用的内置 Discord 扩展和专用的实时 Unity 状态与预览页面。单独持久化的共享开关默认关闭，用于控制是否发布到 Discord。用户可逐项发布项目文件夹名称、Unity 版本和已打开编辑器数量，追加可选的自定义文字，并显示 Unity 图稿与 ALCOMD3 徽章，而不会暴露完整项目路径；启用 Unity 版本时，版本号会显示在活动标题中。打开多个编辑器时，新启动或切换到前台的 Unity 编辑器会成为主编辑器，并在其他应用位于前台时继续保持；不支持前台检测的平台使用最近启动的编辑器。
...
- 简化偏好设置变更：开关和选择器会立即更新，不再显示保存状态、显式回滚或错误通知；每次写入后界面会重新读取持久化值，失败信息仍可在日志中查看。
...
- 将 MCP 工具名称显示和日志视图偏好持久化到 `gui-config.json`，用户设置不再依赖 WebView 本地或会话存储。
- 软件包操作或存储库下载失败时也会计为已处理；最终保存存储库时复用并发预览下载结果；存储库添加后独立启动标准软件包刷新；同时为软件包操作的后端收尾预留最后 1% 的进度，仅在全部工作完成后显示 100%，取消时则保留实际进度。
