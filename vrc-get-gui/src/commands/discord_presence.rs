...
use crate::extensions::UNITY_DISCORD_STATUS_EXTENSION_ID;
use tauri::State;
...
pub fn unity_discord_status(
    ...
    presence.status(
        config
            .get()
            .is_extension_enabled(UNITY_DISCORD_STATUS_EXTENSION_ID),
    )
}
...
pub async fn unity_discord_set_sharing_enabled(
    ...
    let extension_enabled = {
        ...
        let extension_enabled = config.is_extension_enabled(UNITY_DISCORD_STATUS_EXTENSION_ID);
        config.save().await?;
        extension_enabled
    };
