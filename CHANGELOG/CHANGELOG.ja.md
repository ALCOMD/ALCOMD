...
- 既定で有効な組み込み Discord 拡張機能を追加し、Unity の状態とプレビュー専用ページを提供しました。永続化される共有スイッチは既定でオフのままで、Discord への公開を制御します。完全なプロジェクトパスを公開せずに、プロジェクトフォルダー名、Unity バージョン、起動中のエディター数を個別に公開し、任意のカスタムテキスト、Unity アートワーク、ALCOMD3 バッジを追加できます。Unity バージョンを有効にするとアクティビティのタイトルに表示されます。複数のエディターがある場合、新しく起動した、または前面に出した Unity エディターが、別のアプリが前面にある間もプライマリとして維持されます。前面検出に対応しないプラットフォームでは、最後に起動したエディターを使用します。
...
- 設定変更を簡素化し、保存中表示、明示的なロールバック、エラー通知を介さずにスイッチやセレクターを即座に更新するようにしました。書き込み後は永続化された値を再取得し、失敗はログで確認できます。
...
- MCP ツール名表示とログ表示の設定を `gui-config.json` に永続化し、ユーザー設定で WebView のローカルストレージやセッションストレージに依存しないようにしました。
- 失敗したパッケージ操作とリポジトリのダウンロードも処理済みとして数え、並行プレビューダウンロードをリポジトリの最終保存で再利用し、リポジトリ追加後の標準パッケージ更新を独立して開始しました。また、パッケージ操作の最後の 1% をバックエンドの完了処理用に確保し、すべての処理が完了した場合にのみ 100% と表示し、キャンセル時は実際の進捗を維持するようにしました。
