...
ALTER TABLE operation_journal RENAME TO operation_journal_v4;
ALTER TABLE idempotency_records RENAME TO idempotency_records_v4;
ALTER TABLE package_filesystem_journal RENAME TO package_filesystem_journal_v4;
ALTER TABLE package_plans RENAME TO package_plans_v4;
ALTER TABLE operations RENAME TO operations_v4;
...
SELECT * FROM operation_journal_v4;
...
SELECT * FROM idempotency_records_v4;
...
SELECT * FROM package_filesystem_journal_v4;
...
DROP TABLE package_filesystem_journal_v4;
DROP TABLE package_plans_v4;
DROP TABLE operation_journal_v4;
DROP TABLE idempotency_records_v4;
DROP TABLE operations_v4;
...
CREATE TABLE template_plans (
    ...
    owner_principal_id TEXT NOT NULL
        CHECK (length(owner_principal_id) BETWEEN 1 AND 128),
    kind TEXT NOT NULL
        CHECK (kind IN ('import', 'derive', 'create-project')),
    state TEXT NOT NULL
        CHECK (state IN ('unapplied', 'applied')),
