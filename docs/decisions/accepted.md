...
| A-033 | ALCOMD3 v4 official GUI 必须保持与 ALCOMD3 v3 可识别的结构与产品连续性。v3 是整体 shell composition、导航深度与用户语义分组、信息密度、主要工作流空间关系和产品视觉辨识度的参考基线，而不是必须逐项复制的固定导航树。v4 可在有明确用户价值或架构理由时重组个别页面、合并或拆分次级 surface 并引入新能力；Core/domain/API 边界不得自动成为 GUI 一级导航。Material Design 3 现代化组件、交互和视觉语言，但不得把产品变成无关的信息架构。重大变化按 divergence budget 记录并取得项目所有者批准；本决策进一步约束 A-020，不使 A-020 失效 |
