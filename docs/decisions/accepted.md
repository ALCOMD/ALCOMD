...
| A-033 | Official GUI v4 以 ALCOMD3 v3 的宏观 information architecture、navigation model、page grouping 与 major layout 为设计基线。Material Design 3 modernization 改变视觉与组件语言，不改变产品可识别的结构布局；Material adaptive structure 或有意 UX correction 若改变 primary navigation、major hierarchy/content/action placement、list/detail、settings organization 或 package/repository/project workflow shape，必须记录明确 deviation 并取得项目所有者批准。本决策进一步约束 A-020，不使 A-020 失效 |
