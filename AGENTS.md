...
- 早于 3.4.0 的 ALCOMD3 经 `alcomd3.cqmhv.com` 的保留路径 301 到官网 `/api/gui/` 迁移端点并升级到 3.4.0；3.4.0 使用 `alcomd.cqmhv.com/api/v1/updates/` 的 Bridge API。两段链路都必须验证 ALCOMD3 自有 updater 签名，不得复用其他项目的 secret 或未验证 metadata。
