...
- Material Theme 入口属于扩展管理中的内置侧边栏扩展，并参与持久化的侧边栏排序。该扩展
  不能安装或卸载，但可以停用；停用后撤销自定义主题效果并恢复内置的系统响应配色，同时
  保留已保存的主题设置供重新启用时恢复。
...
- 注册表保留写入顺序。内置扩展依次注册为 MCP、主题、日志、Unity Discord 状态；第三方条目按注册顺序追加，
  已有目录条目安装时移动到末尾以体现该次安装顺序。扩展管理直接使用注册表顺序，不再排序。
- Unity Discord 状态扩展默认关闭，使用集中配置的公开 Discord Application ID。其工作线程检测
  本机 Unity 编辑器进程并维护固定优先顺序。新启动的编辑器或检测到位于前台的编辑器会移动到首位；
  其他应用进入前台时保持当前选择。无法检测前台编辑器的平台使用最近启动的编辑器。扩展发送所选编辑器
  的项目文件夹名、Unity 版本、会话时长和已打开编辑器数量，但不发送完整项目路径。扩展启用状态仅控制
  Unity 检测工作线程；页面上另设独立且持久化的
  Discord 共享开关，只控制是否发布状态。关闭共享时立即清除 Discord 活动，但检测和负载预览继续
  运行。页面不得暴露已配置的 Application ID 或完整项目路径；Unity 关闭、扩展停用或 ALCOMD3
  退出时也会清除状态。
