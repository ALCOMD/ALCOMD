...
构建、收集、验证、签名和打包目标平台所需的全部 release-blocker 组件，并产出 Windows
...
- O-003 已关闭，目标平台最低系统、运行库与构建基础镜像已冻结。
- 永久 Windows AppId、签名身份、notarization 身份、更新签名/轮换和真实发布端点已人工批准。
...
.github/workflows/ 中的发行、签名与平台安装测试
...
5. 提供不签名的 `--check`/fixture 路径供 CI 验证，不伪造正式签名成功。
...
1. 生成包含完整组件的 `ALCOMD.app` 与 `ALCOMD_<version>_macos_aarch64.dmg`。
2. 按依赖顺序执行嵌套签名、notarization、stapling 与 Gatekeeper 验证。
...
1. `alcomd-updater` 验证更新 Manifest、目标、版本、完整包签名与组件清单。
2. `alcomd-bootstrap` 协调停止、journal、替换、迁移、健康检查、Commit、恢复和清理。
3. Windows 使用签名完整 Inno 包；macOS 使用完整产品替换流程；AppImage 使用便携更新；DEB
   只通过包管理器更新程序文件。
