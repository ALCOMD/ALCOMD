...
ALTER TABLE operation_journal RENAME TO operation_journal_v6;
ALTER TABLE idempotency_records RENAME TO idempotency_records_v6;
ALTER TABLE package_filesystem_journal RENAME TO package_filesystem_journal_v6;
ALTER TABLE package_plans RENAME TO package_plans_v6;
ALTER TABLE template_plans RENAME TO template_plans_v6;
...
INSERT INTO operations SELECT * FROM operations_v6;
...
INSERT INTO operation_journal SELECT * FROM operation_journal_v6;
...
INSERT INTO idempotency_records SELECT * FROM idempotency_records_v6;
...
INSERT INTO package_plans SELECT * FROM package_plans_v6;
...
INSERT INTO package_filesystem_journal SELECT * FROM package_filesystem_journal_v6;
...
INSERT INTO template_plans SELECT * FROM template_plans_v6;
...
DROP TABLE package_filesystem_journal_v6;
DROP TABLE template_plans_v6;
DROP TABLE package_plans_v6;
DROP TABLE operation_journal_v6;
DROP TABLE idempotency_records_v6;
...
CREATE TABLE backup_restore_plans (
    ...
    preallocated_project_id TEXT NOT NULL UNIQUE CHECK (length(preallocated_project_id) = 36),
    backup_archive_sha256 BLOB NOT NULL CHECK (length(backup_archive_sha256) = 32),
    backup_file_identity BLOB NOT NULL CHECK (length(backup_file_identity) BETWEEN 1 AND 128),
    backup_byte_size INTEGER NOT NULL CHECK (backup_byte_size BETWEEN 1 AND 68719476736),
    backup_format_version INTEGER NOT NULL CHECK (backup_format_version = 1),
    backup_manifest_fingerprint BLOB NOT NULL CHECK (length(backup_manifest_fingerprint) = 32),
    exclude_vpm_packages INTEGER NOT NULL CHECK (exclude_vpm_packages IN (0, 1)),
    excluded_packages_json TEXT NOT NULL CHECK (
        json_valid(excluded_packages_json)
        AND json_type(excluded_packages_json) = 'array'
        AND json_array_length(excluded_packages_json) <= 4096
        AND length(CAST(excluded_packages_json AS BLOB)) <= 1048576
    ),
    ...
    target_must_be_absent INTEGER NOT NULL CHECK (target_must_be_absent = 1),
    expected_unity_project_json TEXT NOT NULL CHECK (
        json_valid(expected_unity_project_json)
        AND json_type(expected_unity_project_json) = 'object'
        AND length(CAST(expected_unity_project_json AS BLOB)) <= 65536
    ),
    plan_fingerprint BLOB NOT NULL CHECK (length(plan_fingerprint) = 32),
    plan_json TEXT NOT NULL CHECK (
        json_valid(plan_json)
        ...
        AND json_type(plan_json, '$.version') = 'integer'
        AND json_extract(plan_json, '$.version') = 1
        AND json_extract(plan_json, '$.backupId') = backup_id
        AND json_extract(plan_json, '$.preallocatedProjectId') = preallocated_project_id
        AND json_extract(plan_json, '$.targetMustBeAbsent') = 1
        AND length(CAST(plan_json AS BLOB)) <= 4194304
    ),
    apply_operation_id TEXT UNIQUE REFERENCES operations(operation_id),
    created_at_ms INTEGER NOT NULL CHECK (created_at_ms BETWEEN 0 AND 9223372036854775807),
...
CREATE INDEX backup_restore_plans_owner_created
    ON backup_restore_plans(owner_principal_id, created_at_ms DESC, plan_id DESC);
CREATE INDEX backup_restore_plans_backup_created
    ON backup_restore_plans(backup_id, created_at_ms DESC, plan_id DESC);
...
WHEN NOT (
    ...
    AND OLD.plan_id = NEW.plan_id
    AND OLD.owner_principal_id = NEW.owner_principal_id
    AND OLD.backup_id = NEW.backup_id
    AND OLD.preallocated_project_id = NEW.preallocated_project_id
    AND OLD.backup_archive_sha256 = NEW.backup_archive_sha256
    AND OLD.backup_file_identity = NEW.backup_file_identity
    AND OLD.backup_byte_size = NEW.backup_byte_size
    AND OLD.backup_format_version = NEW.backup_format_version
    AND OLD.backup_manifest_fingerprint = NEW.backup_manifest_fingerprint
    AND OLD.exclude_vpm_packages = NEW.exclude_vpm_packages
    AND OLD.excluded_packages_json = NEW.excluded_packages_json
    ...
    AND OLD.target_must_be_absent = NEW.target_must_be_absent
    AND OLD.expected_unity_project_json = NEW.expected_unity_project_json
    AND OLD.plan_fingerprint = NEW.plan_fingerprint
    AND OLD.plan_json = NEW.plan_json
    AND OLD.created_at_ms = NEW.created_at_ms
    AND (SELECT kind FROM operations WHERE operation_id = NEW.apply_operation_id) = 'backups.restore'
)
BEGIN
    SELECT RAISE(ABORT, 'backup restore plan is immutable');
END;

CREATE TRIGGER backup_restore_plans_no_delete
BEFORE DELETE ON backup_restore_plans
BEGIN
    SELECT RAISE(ABORT, 'backup restore plans are durable');
END;

CREATE TABLE backup_restore_filesystem_journal (
    operation_id TEXT NOT NULL REFERENCES operations(operation_id) ON DELETE CASCADE,
    step INTEGER NOT NULL CHECK (step BETWEEN 1 AND 9223372036854775807),
    plan_id TEXT NOT NULL REFERENCES backup_restore_plans(plan_id),
    preallocated_project_id TEXT NOT NULL CHECK (length(preallocated_project_id) = 36),
    phase TEXT NOT NULL CHECK (phase IN (
        'accepted', 'archive_verified', 'extracting', 'staging_complete',
        'publish_intent', 'target_published', 'project_registry_commit_intent',
        'state_committed'
    )),
    state TEXT NOT NULL CHECK (state IN ('intent', 'completed')),
    target_parent_identity BLOB NOT NULL CHECK (length(target_parent_identity) BETWEEN 1 AND 128),
    target_identity BLOB CHECK (
        target_identity IS NULL OR length(target_identity) BETWEEN 1 AND 128
    ),
    project_fingerprint BLOB CHECK (
        project_fingerprint IS NULL OR length(project_fingerprint) = 32
    ),
    evidence_json TEXT NOT NULL CHECK (
        json_valid(evidence_json) AND json_type(evidence_json) = 'object'
...
CREATE INDEX backup_restore_filesystem_journal_recovery
    ON backup_restore_filesystem_journal(operation_id, step DESC);

CREATE TRIGGER backup_restore_filesystem_journal_validate_insert
BEFORE INSERT ON backup_restore_filesystem_journal
WHEN (SELECT kind FROM operations WHERE operation_id = NEW.operation_id) IS NOT 'backups.restore'
     OR (SELECT apply_operation_id FROM backup_restore_plans WHERE plan_id = NEW.plan_id)
        IS NOT NEW.operation_id
     OR (SELECT preallocated_project_id FROM backup_restore_plans WHERE plan_id = NEW.plan_id)
        IS NOT NEW.preallocated_project_id
BEGIN
    SELECT RAISE(ABORT, 'backup restore journal ownership mismatch');
END;

CREATE TRIGGER backup_restore_filesystem_journal_no_update
BEFORE UPDATE ON backup_restore_filesystem_journal
BEGIN
    SELECT RAISE(ABORT, 'backup restore journal is append-only');
END;
